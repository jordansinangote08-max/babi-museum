...
const ROOMS = [
  ...
  { title: 'Our Songs', subtitle: 'Two songs that belong inside this collection.' },
...
function render() {
  ...
  bindEvents();
  if (state.view === 'room') setupRoomObservers();
}
...
function renderRoomBody(i) {
  switch (i) {
    ...
    case 5:
      return `${artifact('DATE ARCHIVE', 'Hoppers', `<p>A cinema date that became special for the simplest reason: you were together.</p>`)}
    ...
    case 14:
      return `<div class="future-grid">${FUTURE.map(([title, body, note]) => `<section class="future-piece"><div class="kicker">FUTURE ARTIFACT</div><h2>${escapeHTML(title)}</h2><div class="future-frame">RESERVED</div><p>${escapeHTML(body)}</p>${note ? `<small>${escapeHTML(note)}</small>` : ''}</section>`).join('')}</div>
...
function songCard(key, title, src) {
  const playing = state.activeSong === key && !songAudio.paused;
  return `<button class="song-card ${playing ? 'playing' : ''}" data-song="${key}" data-src="${src}"><div><strong>${escapeHTML(title)}</strong><span>${playing ? 'Pause' : 'Play inside the museum'}</span></div></button>`;
}
...
function bindEvents() {
  document.querySelectorAll('[data-action]').forEach(el => {
    ...
    if (action === 'open-settings') el.onclick = () => { state.drawer = true; render(); };
  ...
  document.querySelectorAll('[data-hall-room]').forEach(btn => {
    btn.onclick = () => { const idx = Number(btn.dataset.hallRoom); if (idx !== state.room) return; state.view = 'room'; markVisited(idx); tone('chime'); render(); window.scrollTo(0,0); };
  });
  setupHallPan();
...
function nextRoom() {
  if (state.room >= ROOMS.length - 1) return;
  state.room += 1;
  state.overlayLetter = null;
  state.map = false;
  state.drawer = false;
...
function toggleSong(key, src) {
  if (state.activeSong === key && !songAudio.paused) {
    state._manualSongPause = true;
    songAudio.pause();
    return;
  }
  pauseBackground();
  ...
  songAudio.src = src;
  songAudio.currentTime = 0;
  songAudio.play().catch(() => { state.activeSong = null; resumeBackground(); });
  render();
}
