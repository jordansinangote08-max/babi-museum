...
const ROOMS = [
  ...
  { title: 'Our Photos', subtitle: 'Five pieces from the current collection.' },
...
function roomScreen() {
  ...
  return `<main class="museum-main ambient-${state.room}" data-room="${state.room}"><div class="room-ambience" aria-hidden="true"><i></i><i></i><i></i></div>
    ...
    ${body}
    ${roomDecor(state.room)}
  </main>`;
}

function roomDecor(i) {
  const scenes = [
    ['FOUND OBJECTS','watch','A tiny watch pedestal','instagram','A disappearing story frame'],
    ['CAFE CORNER','cup','Steam, cookies, and coffee','menu','A little table for two'],
    ['STREET ARCHIVE','lamp','A warm street lamp','bag','Banana bread & dragon fruit'],
    ['CLOSURE DESK','stamp','DENIED stamp','rope','Museum rope: still open'],
    ['WARDROBE CASE','shirt','The matching piece','tag','Official gaya-gaya evidence'],
    ['DATE DISPLAY','ticket','Cinema ticket','strip','Photobooth strip'],
    ['TRAVEL SHELF','mountain','Baguio skyline','camera','Three travel photos'],
    ['TIME INSTALLATION','calendar','January 25','calendar','July 25'],
    ['PHOTO SALON','frame','Private collection','film','Six photographs'],
    ['CATALOG TABLE','cards','10 reasons','heart','One very lucky boyfriend'],
    ['QUIZ DESK','question','Certified Babi','badge','Credentials renewed'],
    ['LISTENING ROOM','record','Now spinning','note','Songs that remind me of you'],
    ['LETTER TABLE','envelope','Open when…','seal','Three private envelopes'],
    ['PRIVATE CORRESPONDENCE','letter','To my Babi','seal','From Jordan'],
    ['FUTURE WALL','emptyframe','Reserved space','home','Memories still to come'],
    ['CLASSIFIED CASE','door','One hidden room','key','One visitor only'],
    ['ARCHIVE DESK','book','Permanent guestbook','pen','Signed by Jirby']
  ];
  const s = scenes[i] || scenes[0];
  return `<section class="room-decor room-decor-${i}" aria-label="Decorative museum installation">
    <div class="room-decor-plaque"><span>${escapeHTML(s[0])}</span></div>
    <div class="room-decor-stage">
      <div class="decor-piece decor-${s[1]}"><span class="decor-object"></span><small>${escapeHTML(s[2])}</small></div>
      <div class="decor-divider"></div>
      <div class="decor-piece decor-${s[3]}"><span class="decor-object"></span><small>${escapeHTML(s[4])}</small></div>
    </div>
  </section>`;
}
...
function renderRoomBody(i) {
  switch (i) {
    ...
    case 8:
      return `${photoGallery()}${doorHTML(state.galleryIndex >= 5, 'Reasons I Love You')}`;
...
function photoGallery() {
  const slides = [1,2,3,4,5].map((n, idx) => `<section class="gallery-slide" data-gallery-slide="${idx}">
    <img src="./assets/photos/photo-${n}.jpeg" alt="Museum photograph ${n}" draggable="false" data-palette-image />
    <div class="gallery-count"><span>${String(n).padStart(2,'0')} / 05</span><span>PRIVATE COLLECTION</span></div>
  </section>`).join('');
  return `<div class="gallery-shell" data-gallery-shell><div class="gallery-track" data-gallery-track>${slides}<section class="gallery-slide" data-gallery-slide="5"><div class="gallery-reserved"><div><div class="kicker">06 / ?</div><h2>Reserved for our next favorite photo.</h2></div></div><div class="gallery-count"><span>06 / ?</span><span>TO BE CONTINUED</span></div></section></div></div><div class="swipe-hint">Swipe manually through the collection</div>`;
}
...
function finalScreen() {
  return `<section class="final-screen"><div><h1>MUSEUM OF US</h1><p>Thank you for visiting, Babi.</p><b>Curated by Jordan</b><small>For one visitor only.</small></div></section>`;
}
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-hall-room]').forEach(btn => {
    btn.onclick = () => { const idx = Number(btn.dataset.hallRoom); if (idx !== state.room) return; state.view = 'room'; markVisited(idx); if (idx === 11) state.musicUnlocked = true; tone('chime'); render(); window.scrollTo(0,0); };
  });
  setupHallPan();
